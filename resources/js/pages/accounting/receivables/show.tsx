...
import { ArrowLeft, DollarSign, Printer, AlertTriangle } from 'lucide-react';
...
export default function ShowReceivable({ receivable }: Props) {
    ...
    const handlePayment = (e: React.FormEvent) => {
        e.preventDefault();
        post(`/accounting/receivables/${receivable.id}/payment`, {
            onSuccess: () => {
    ...
    const handleMarkAsBadDebt = () => {
        if (confirm(t('accounting.confirm_bad_debt'))) {
            router.post(`/accounting/receivables/${receivable.id}/bad-debt`);
        }
    };
    ...
    return (
        <AppLayout breadcrumbs={[
            ...
            { title: receivable.invoice_number, href: `/accounting/receivables/${receivable.id}` },
        ]}>
            <Head title={receivable.invoice_number} />
                ...
                <div className="flex items-center justify-between print:hidden">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-muted-foreground">{receivable.customer?.name}</p>
                        </div>
                    </div>
                    <div className="flex gap-2">
                        ...
                        {receivable.balance_due > 0 && receivable.status !== 'bad_debt' && (
                            <>
                                <Button variant="outline" onClick={handleMarkAsBadDebt} className="text-red-600">
                                    <AlertTriangle className="h-4 w-4 mr-2" />
                                    {t('accounting.mark_bad_debt')}
                                </Button>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p className="font-medium">
                                        {new Date(receivable.invoice_date).toLocaleDateString(currentLanguage === 'my' ? 'my-MM' : 'en-US')}
                                    </p>
                                </div>
                                <div>
                                    <Label className="text-muted-foreground">{t('accounting.due_date')}</Label>
                                    <p className="font-medium">
                                        {new Date(receivable.due_date).toLocaleDateString(currentLanguage === 'my' ? 'my-MM' : 'en-US')}
                                    </p>
                                </div>
                            </div>
                            {receivable.order && (
                                <div>
                                    <Label className="text-muted-foreground">{t('orders.order_number')}</Label>
                                    <Link href={`/orders/${receivable.order.id}`} className="font-medium text-primary hover:underline block">
                                        {receivable.order.order_number}
                                    </Link>
                                </div>
                            )}
                    ...
                    <Card className="backdrop-blur-sm bg-background/80 border-sidebar-border/70">
                        ...
                        <CardContent className="space-y-4">
                            <div className="flex justify-between">
                                ...
                                <span>{formatCurrency(receivable.subtotal, currentLanguage === 'my' ? 'my-MM' : 'en-US')}</span>
                            </div>
                            <div className="flex justify-between">
                                <span>{t('accounting.tax')}</span>
                                <span>{formatCurrency(receivable.tax_amount, currentLanguage === 'my' ? 'my-MM' : 'en-US')}</span>
                            </div>
                            <div className="flex justify-between font-bold border-t pt-2">
                                <span>{t('common.total')}</span>
                                <span>{formatCurrency(receivable.total_amount, currentLanguage === 'my' ? 'my-MM' : 'en-US')}</span>
                            </div>
                            <div className="flex justify-between text-green-600">
                                <span>{t('accounting.paid')}</span>
                                <span>{formatCurrency(receivable.paid_amount, currentLanguage === 'my' ? 'my-MM' : 'en-US')}</span>
                            </div>
                            <div className="flex justify-between font-bold text-lg border-t pt-2">
                                <span>{t('accounting.balance')}</span>
                                <span className={receivable.balance_due > 0 ? 'text-red-600' : 'text-green-600'}>
                                    {formatCurrency(receivable.balance_due, currentLanguage === 'my' ? 'my-MM' : 'en-US')}
                                </span>
                            </div>
                        </CardContent>
                    </Card>
