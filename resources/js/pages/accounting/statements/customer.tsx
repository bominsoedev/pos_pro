...
export default function CustomerStatement({ customers, customer, statement, filters }: Props) {
    const { t, currentLanguage } = useTranslation();
    const locale = currentLanguage === 'my' ? 'my-MM' : 'en-US';
    
    const [customerId, setCustomerId] = useState(filters.customer_id?.toString() || '');
    const [startDate, setStartDate] = useState(filters.start_date);
    const [endDate, setEndDate] = useState(filters.end_date);

    const handleGenerate = () => {
        router.get('/accounting/statements/customer', {
    ...
    return (
        <AppLayout breadcrumbs={[
            ...
            { title: t('accounting.statements'), href: '/accounting/statements/customer' },
            { title: t('accounting.customer_statement'), href: '/accounting/statements/customer' },
        ]}>
            <Head title={t('accounting.customer_statement')} />
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-6">
                <div className="flex items-center justify-between print:hidden">
                    <div>
                        <h1 className="text-2xl font-bold flex items-center gap-2">
                            <FileText className="h-6 w-6" />
                            {t('accounting.customer_statement')}
                        </h1>
                        <p className="text-muted-foreground">{t('accounting.customer_statement_desc')}</p>
                    </div>
                    {statement && (
                        ...
                        <CardHeader className="print:pb-2">
                            <div className="flex justify-between items-start">
                                <div>
                                    ...
                                    <p className="text-lg font-bold mt-2">{customer.name}</p>
                                </div>
                ...
                {!statement && (
                    <Card className="backdrop-blur-sm bg-background/80 border-sidebar-border/70">
                        <CardContent className="py-12 text-center text-muted-foreground">
                            {t('accounting.select_customer_prompt')}
                        </CardContent>
                    </Card>
                )}
            </div>
